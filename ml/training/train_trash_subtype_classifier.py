#!/usr/bin/env python3
"""
Fine-tune EfficientNet-B0 for TRASH subtype classification (Stage 2 pipeline).

Classes: CONSTRUCTION | ELECTRONIC | HAZARDOUS | HOUSEHOLD | MEDICAL | ORGANIC | RECYCLABLE

Dataset layout (relative to --data-root):
  images/train/CONSTRUCTION/   *.jpg / *.png
  images/train/ELECTRONIC/
  images/train/HAZARDOUS/
  images/train/MEDICAL/
  images/train/ORGANIC/
  images/train/RECYCLABLE/
  images/val/...               (optional — auto-split from train if missing)

No bbox annotation needed — just images in class folders (ImageFolder format).

Output:
  ml/weights/trash_subtype_classifier.pt
  Then set TRASH_SUBTYPE_MODEL_PATH=ml/weights/trash_subtype_classifier.pt in .env

Usage (from repo root):
  uv run python ml/training/train_trash_subtype_classifier.py
  uv run python ml/training/train_trash_subtype_classifier.py --epochs 100 --batch-size 32
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Fine-tune EfficientNet-B0 for TRASH subtype classification."
    )
    p.add_argument(
        "--data-root",
        default=str(_PROJECT_ROOT / "ml" / "training" / "data" / "trash_subtype"),
        help="Root with images/train/{CONSTRUCTION,ELECTRONIC,...}/ sub-folders.",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.15,
        help="If images/val/ is missing, split this fraction from train automatically.",
    )
    p.add_argument(
        "--output",
        default=str(_PROJECT_ROOT / "ml" / "weights" / "trash_subtype_classifier.pt"),
...
def main() -> int:
    ...
    if not train_dir.is_dir():
        raise SystemExit(
            ...
            f"Create images/train/{{{'|'.join(_CLASSES)}}}/ and add images."
        )
    ...
    if not val_has_data:
        _log(f"[info] No val split found — splitting {args.val_fraction:.0%} from train.")
    ...
    _log(f"TrashSubtypeClassifier  classes={len(train_ds.classes)}  device={device}")
    _log(f"{'dataset':>10}  train={len(train_ds)}  val={len(val_ds)}")
    _log(f"optimizer  AdamW  lr={args.lr}  scheduler=CosineAnnealing")
    _log("")
    ...
    _log("")
    _log("--- Paste vao .env ---------------------------------")
    _log(f"  TRASH_SUBTYPE_MODEL_PATH={output_path}")
    _log(f"{'='*60}")
    return 0
